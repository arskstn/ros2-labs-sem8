...
'''
используется абсолютная позиция по варианту 3. базируется на лаб 3.

лаб 7, вариант 3: публикует кастомное сообщение TurtleStats с полями:
    distance_traveled — суммарный пройденный путь (накапливается из разности позиций)
    max_speed         — максимальная линейная скорость за всё время
    min_speed         — минимальная ненулевая линейная скорость за всё время
скорость берётся из поля linear_velocity топика /turtle1/pose (turtlesim пишет сам).
...
from my_turtle_interface.msg import TurtleStats


class TurtleFigureTriangleLab7(Node):
    def pose_callback(self, msg):
        ...
        #накапливаем пройденный путь из разности позиций
        dx = msg.x - self.current_x
        dy = msg.y - self.current_y
        self.distance_traveled += (dx**2 + dy**2)**(1/2)

        #отслеживаем макс и мин скорость из поля linear_velocity топика pose
        if linear_vel > 0.0:
            if linear_vel > self.max_speed:
                self.max_speed = linear_vel
            if linear_vel < self.min_speed:
                self.min_speed = linear_vel
    ...
    def __init__(self):
        super().__init__('turtle_figure_triangle_lab7')
        self.publisher_ = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)
        self.stats_publisher_ = self.create_publisher(TurtleStats, '/turtle1/stats', 10)
        ...
        #публикация статистики 1 раз в секунду
    ...
    def publish_stats(self):
        ...
        #если движение ещё не началось — мин скорость репортим как 0.0
        msg.min_speed = self.min_speed if self.min_speed != float('inf') else 0.0
        self.stats_publisher_.publish(msg)
        self.get_logger().info(
            f'Stats: distance={msg.distance_traveled:.3f}, '
            f'max_speed={msg.max_speed:.3f}, min_speed={msg.min_speed:.3f}'
        )
